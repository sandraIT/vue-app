	...
	class InsightCore {
		...
		public function set_info() {
			self::$info = array(
				...
				'child'   => 'http://api.insightstud.io/update/thememove/thememove-child.zip',
		...
		public function add_extra_fields_for_contactmethods( $contactmethods ) {
			if ( get_theme_support( 'insight-user-social-networks' ) ) {
				...
				$default = array(
					...
					array(
						'name'  => 'google_plus',
						'label' => esc_html__( 'Google+', 'insight-core' ),
					),
		...
		// Ajax get changelogs content
		function ajax_get_changelogs() {
			echo self::get_changelogs();
			die();
		}
		...
		public static function check_purchase_code( $code ) {
			$request = wp_remote_get( 'http://api.insightstud.io/purchase/tf.php?code=' . $code, array( 'timeout' => 120 ) );

				...
				<div class="box-body">
					...
					<script>
						jQuery( document ).ready( function() {
							data = {
								action: 'insight_core_get_changelogs',
								ic_nonce: ic_vars.ic_nonce
							};
							jQuery.post( ic_vars.ajax_url, data, function( response ) {
								if ( response != '' ) {
									jQuery( '#changelogs-content' ).html( '<div class="changelogs-wrap"><table class="wp-list-table widefat striped table-change-logs changelogs"><thead><tr><th>Time</th><th>Version</th><th>Note</th></tr></thead><tbody>' + response + '</tbody></table></div>' );
								} else {
									jQuery( '#changelogs-content' ).html( 'The changelogs not found!' );
								}
							} );
						} );
					</script>
